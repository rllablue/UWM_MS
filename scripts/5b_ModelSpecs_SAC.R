...
################################################## try at multiple distances; justify for supp materials
max_disp_km <- 20
...
data_dir$RLL_col <- data_dir$RLL_col %>%
  ...
  mutate(
    sac_kernel = as.numeric(scale(kernel_connectivity_std))
    ) %>%
  dplyr::select(-kernel_connectivity_std)
...
# pa subsets
sac2_glm_models <- lapply(names(pasub_glm_models), function(nm) { # or pasub_glm_models
  
  glm(
    formula = update(
      formula(pasub_glm_models[[nm]]),
      . ~ . + sac_kernel
    ),
    data = data_dir[[nm]],
    family = binomial
...
names(sac2_glm_models) <- names(pasub_glm_models)

sac2_glm_summaries <- lapply(sac2_glm_models, summary)
sac2_glm_summaries
...
### SCRIPT NOTE: SAC covariate term was derived, assessed, and selected from following workflow. ###
