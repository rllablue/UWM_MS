...
assign_atlas_period <- function(date) {
  case_when(
...
# Set auk path
auk_set_ebd_path("Z:/Users/lablue/Wisconsin/WIBBA/R_Projects/WIBBA/data/ebird/ebird_raw", overwrite = TRUE)
...
observations_base <- read_ebd("data/ebird/ebird_filtered/wibba_ebd_base_filtered.txt") %>% 
  filter_atlas_data()

observations_sensitive <- read_ebd("data/ebird/ebird_filtered/wibba_ebd_sensitive_filtered.txt") %>% 
  filter_atlas_data()
...
checklists_filt <- checklists_filt %>% 
  mutate(atlas_period = assign_atlas_period(observation_date))
observations_filt <- observations_filt %>% 
  mutate(atlas_period = assign_atlas_period(observation_date))
...
sr_summary <- observations_filt %>%
  ...
  left_join(dplyr::select(checklist_summary, atlas_block, checklists_Atlas1, checklists_Atlas2), by = "atlas_block") %>%
  mutate(
  ...
  dplyr::select(atlas_block, sr_total, sr_Atlas1, sr_Atlas2, sr_diff, sr_diff_z)
...
wibba_summary_full <- blocks_all %>%
  ...
  dplyr::select(-mins_Atlas1_supp)

wibba_summary_full <- wibba_summary_full %>%
...
## -- FILTER COMPARABLE MAPS/SHP FILES -- ##
